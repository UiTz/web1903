<template>
	<div>
		<el-button type="text" style="color:rgb(177, 105, 57);margin-top:15px;" @click="dialogFormVisible = true">注册
		</el-button>
		<el-dialog :modal-append-to-body='false' id="myclass1" :before-close="handleclose" title="用户注册"
			...
			<el-form :model="form" status-icon :rules="rules" ref="form" label-width="80px" class="demo-ruleForm">
				...
				<el-form-item label="昵称" prop="user_name">
					<el-input v-model="form.user_name" auto-complete="off" placeholder="请输入昵称"></el-input>
				</el-form-item>
	...
	export default {
		data () {
			...
			return {
				...
				rules: {
					...
					user_name: [
						{ required: true, message: "用户名不能为空", trigger: 'blur' },
						{ pattern: /^[\u4e00-\u9fa5_a-zA-Z0-9]{3,8}$/, message: "长度3到8个字符, 仅中文，英文字母和数字", trigger: 'blur' },
					],
					upwd: [
		...
		methods: {
			submitForm (formName) {
				...
				var obj = { uname: u, user_name: un, upwd: p, email: e, phone: t };
