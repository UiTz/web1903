...
app.post('/user/api/register', (req, res) => {
	let r = req.body;
	let { uname, upwd, email, phone, user_name } = r;
	if (uname === '') {
		res.json("用户名不能为空");
		return;
	}
	...
	let sql = 'INSERT INTO pj_user (uname,upwd,email,phone,user_name,regster_date) VALUES (?,md5(?),?,?,?,NOW())';
	query(sql, [uname, upwd, email, phone, user_name], (err, result) => {
		if (err) throw err;
...
app.post('/user/api/login', (req, res) => {
	...
	let sql = 'SELECT uid,user_name FROM pj_user WHERE uname = ? AND upwd = md5(?)';
...
// 开启端口监听
app.listen(5050, () => {
	console.log("服务器开启成功: http://localhost:5050");
});
